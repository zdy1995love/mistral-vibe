...
| 2026-04-28 | Selective context compaction: `/compact --micro` and `MicroCompactMiddleware` clear stale tool-result content while preserving the message stream and tool-call structure; configurable via `micro_compact_ratio` (default `0.7`) and `micro_keep_last` (default `2`) | Avoid the "lost critical context after `/compact`" problem of full summarization-based compaction; only the easily-regeneratable parts (file reads, shell output, grep results, web fetches) get cleared |
| 2026-04-28 | Client-side `MistralToolCallTextExtractor` for vLLM streaming tool-call parser; opt-in per provider via `parse_text_tool_calls` | When a self-hosted vLLM server is started with **both** `--tool-call-parser mistral` and `--reasoning-parser mistral`, the serving layer routes every chunk through the reasoning parser until reasoning ends. If the model skips `[THINK]...[/THINK]` and emits a tool call directly, `[TOOL_CALLS]<name>{<args>}` leaks into `delta.content`. Extractor reassembles structured `tool_calls` on the client. **Server-side workaround**: drop `--reasoning-parser mistral` from the vLLM launch and let `ThinkTagExtractor` handle reasoning text instead |
| 2026-04-29 | Plan mode redesign: dispatch-layer gate (`mutates_state`), read-only bash commands allowed (`ls`/`find`/`grep`/`git log`/…), plan files at **project-local** `<cwd>/.vibe/plans/`, `ExitPlanMode` forks to a fresh dev context with the plan as seed (clears history, regenerates session id, switches profile), `/plan` slash command + `[PLAN]` status chip + shift+tab cycle, periodic system reminders | Make plan mode a real "planning sub-session that produces a durable artifact and hands off to a clean implementation context" instead of a same-context toggle. Aligns with Claude Code's clear-context exit semantics |
| 2026-04-29 | Output styles: `/style` slash command (list / switch), three built-in styles (`default` / `concise` / `learner`), user-defined styles via `~/.vibe/prompts/styles/NAME.md` (case-insensitive `.md`, BOM-tolerant), `output_style` config field | Let users tune the assistant's response register without rewriting the system prompt. Default style preserves byte-equivalence with pre-feature prompt assembly so pre-existing setups are unaffected |
