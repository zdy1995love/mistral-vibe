...
- **Superpowers**: 14 bundled skills (invoked as `/superpowers-<name>`: `brainstorming`, `writing-plans`, `systematic-debugging`, `test-driven-development`, `requesting-code-review`, …) plus a general-purpose subagent, vendored from [obra/superpowers](https://github.com/obra/superpowers) (MIT, see `LICENSE.upstream`), shipped inside `vibe/core/skills/builtins/superpowers/` so they work out of the box. **In practice this constrains the workflow remarkably well** — brainstorm → plan → TDD → review tends to actually happen instead of devolving into "let me just patch this." Trade-off: once a skill is invoked, its full `SKILL.md` (often several hundred lines) lands in context, and running a full flow that touches several skills will eat into the budget on local models with sub-100k context.
...
| ---------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- | ---------------- |
| 2026-05-09 | Merged upstream v2.9.5 into dev: `/loop` slash command (scheduled in-session prompts via `ScheduledLoopRunner`), `default_agent` config, parallel-safe history, cause-chain-aware `_is_non_retryable_error`, teleport telemetry shims (kept hard-disabled in fork). Superpowers skills re-namespaced as `superpowers-<name>` to avoid colliding with `/loop` and future builtin slash commands | Catch up to upstream baseline; shrink the long-term conflict surface | — |
| 2026-05-07 | `/stat` command + per-turn timeline (cache/input/output/cost), replacing markdown `/status`; cache-token plumbing through `LLMUsage` for generic / anthropic / openai-responses backends | Make KV-cache effectiveness visible — biggest factor in long-session cost on local vLLM | Kept |
| 2026-05-01 | `agent_loop` sanitizes truncated `tool_call.arguments` JSON before persisting                            | Truncated JSON args broke session resume with a parse error                                              | Kept             |
| 2026-05-01 | Streaming render perf: coalesce `Markdown.append` writes; offload session-log `fsync` off the render path | Long outputs stalled the UI and `fsync` blocked the render loop                                          | Kept             |
| 2026-05-01 | Thinking field collapsed to two levels: `reasoning_effort` field and `/reasoning` slash command removed in favor of upstream's `thinking` field, narrowed to `off` / `high`; `set_thinking` now writes the linked temperature back to the active model (`off`→`0.3`, `high`→`0.7`). `ThinkTagExtractor`, `send_thinking_blocks`, and CLI expand-by-default are kept | vLLM-only setups in practice need just on/off; multi-level is noise. Temperature linking moves from the slash command into the config layer so all entry points inherit it | Folded upstream  |
| 2026-05-01 | Merged upstream v2.9.0–v2.9.3 into dev: hooks, scratchpad, `--trust`, MCP/Connector OAuth, headless mode, config-layer abstraction, `session_id` module, `find` allowlist + `-exec` safety, `nuage`→`vibe_code` rename, `auto_approve`→`bypass_tool_permissions`, etc. | Catch up to upstream baseline; shrink the long-term conflict surface                                     | —                |
| 2026-04-29 | `/agents` slash command + `AgentsPickerApp` bottom-app (list / preview / edit / view-full); `AgentManager.reload_from_disk` preserves runtime-registered agents across re-discovery | Let agent configs be edited and hot-reloaded inside the session without restart                         | Kept             |
| 2026-04-29 | Output styles: `/style` slash command (list / switch), three built-in styles (`default` / `concise` / `learner`), user-defined styles via `~/.vibe/prompts/styles/NAME.md` (case-insensitive `.md`, BOM-tolerant), `output_style` config field | Let users tune the assistant's response register without rewriting the system prompt. Default style preserves byte-equivalence with pre-feature prompt assembly so pre-existing setups are unaffected | Kept             |
| 2026-04-29 | Plan mode redesign: dispatch-layer gate (`mutates_state`), read-only bash commands allowed (`ls`/`find`/`grep`/`git log`/…), plan files at **project-local** `<cwd>/.vibe/plans/`, `ExitPlanMode` forks to a fresh dev context with the plan as seed (clears history, regenerates session id, switches profile), `/plan` slash command + `[PLAN]` status chip + shift+tab cycle, periodic system reminders | Make plan mode a real "planning sub-session that produces a durable artifact and hands off to a clean implementation context" instead of a same-context toggle. Aligns with Claude Code's clear-context exit semantics | Kept             |
| 2026-04-28 | Client-side `MistralToolCallTextExtractor` for vLLM streaming tool-call parser; opt-in per provider via `parse_text_tool_calls` | When a self-hosted vLLM server is started with **both** `--tool-call-parser mistral` and `--reasoning-parser mistral`, the serving layer routes every chunk through the reasoning parser until reasoning ends. If the model skips `[THINK]...[/THINK]` and emits a tool call directly, `[TOOL_CALLS]<name>{<args>}` leaks into `delta.content`. Extractor reassembles structured `tool_calls` on the client. **Server-side workaround**: drop `--reasoning-parser mistral` from the vLLM launch and let `ThinkTagExtractor` handle reasoning text instead | Kept             |
| 2026-04-28 | Selective context compaction: `/compact --micro` and `MicroCompactMiddleware` clear stale tool-result content while preserving the message stream and tool-call structure; configurable via `micro_compact_ratio` (default `0.7`) and `micro_keep_last` (default `2`) | Avoid the "lost critical context after `/compact`" problem of full summarization-based compaction; only the easily-regeneratable parts (file reads, shell output, grep results, web fetches) get cleared | Kept             |
| 2026-04-26 | Client-side `ThinkTagExtractor` for vLLM streaming parser           | Support self-hosted vLLM models that emit `<think>` inline instead of as a separate `reasoning` field   | Kept             |
| 2026-04-26 | End-to-end reasoning support: `reasoning_effort` config, `/reasoning` runtime toggle, past reasoning replayed as thinking blocks, CLI expands reasoning by default | Make reasoning a first-class, controllable feature so multi-turn chats keep continuity of thought       | Folded upstream  |
...
- **`plan`**: Read-only profile for exploration and planning. Read tools (`grep`, `read_file`) and read-only `bash` commands (`ls`, `find`, `git log`, …) auto-approve; writing is allowed only to the plan file under `<cwd>/.vibe/plans/`. The LLM authors a plan, calls `ExitPlanMode`, and (on approval) forks into a fresh dev context with the plan as the seed message. Toggle with `/plan` or shift+tab; `[PLAN]` shows in the status bar.
