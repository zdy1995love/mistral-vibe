...
- **02-namespace-and-prose.py** — rewrite `superpowers:<name>` plugin prefix to `superpowers-<name>` (vibe parser disallows `:` in skill names; the fork keeps the namespace to avoid collisions with vibe builtins like `/loop`) + targeted prose fixes for executing-plans, using-git-worktrees, writing-skills, using-superpowers.
