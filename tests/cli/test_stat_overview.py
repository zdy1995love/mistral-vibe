...
def test_overview_renders_session_and_last_turn_columns() -> None:
    ...
    stats.turns.append(TurnRecord(
        index=12, prompt_tokens=1024, cached_tokens=1016,
        completion_tokens=42, duration=1.8, started_at=0.0,
    ))
...
def test_overview_no_max_context_skips_bar() -> None:
    stats = AgentStats(
        ...
        session_prompt_tokens=500, session_completion_tokens=10,
    )
    stats.turns.append(TurnRecord(
        index=1, prompt_tokens=500, cached_tokens=0,
        completion_tokens=10, duration=0.5, started_at=0.0,
    ))
    out = _render(stats, max_context=0)
    assert "500" in out
    assert "/" not in out.split("Context")[1].split("\n")[0]  # no denominator on context line
