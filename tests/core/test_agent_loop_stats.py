...
def test_update_stats_records_cached_and_appends_turn_record() -> None:
    ...
    usage = LLMUsage(
        prompt_tokens=2340, completion_tokens=64, cached_prompt_tokens=1820
    )
...
def test_update_stats_accumulates_session_cached_across_turns() -> None:
    ...
    a.stats.steps = 1
    a._update_stats(
        LLMUsage(prompt_tokens=520, completion_tokens=18, cached_prompt_tokens=0), 0.6
    )
    a.stats.steps = 2
    a._update_stats(
        LLMUsage(prompt_tokens=1210, completion_tokens=32, cached_prompt_tokens=480),
        0.9,
    )
...
def test_compaction_turn_does_not_steal_prior_turn_tools() -> None:
    ...
    assert a.stats.turns[1].tools == [], (
        f"compaction TurnRecord captured prior turn's tools: {a.stats.turns[1].tools}"
    )
