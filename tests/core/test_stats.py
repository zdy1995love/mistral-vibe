...
def test_turn_record_tools_list_is_mutable() -> None:
    ...
    t = TurnRecord(
        index=1, prompt_tokens=0, cached_tokens=0,
        completion_tokens=0, duration=0.0, started_at=0.0,
    )
    t.tools.append("read")
    assert t.tools == ["read"]
...
def test_agent_stats_round_trip_preserves_turns() -> None:
    ...
    s.turns.append(TurnRecord(
        index=1, prompt_tokens=520, cached_tokens=0,
        completion_tokens=18, duration=0.6, started_at=0.0,
    ))
    s.turns.append(TurnRecord(
        index=2, prompt_tokens=1210, cached_tokens=480,
        completion_tokens=32, duration=0.9, started_at=1.0, tools=["read"],
    ))
    dumped = s.model_dump()
...
def test_agent_stats_create_fresh_starts_empty_turns() -> None:
    ...
    prev.turns.append(TurnRecord(
        index=1, prompt_tokens=999, cached_tokens=0,
        completion_tokens=10, duration=1.0, started_at=0.0,
    ))
