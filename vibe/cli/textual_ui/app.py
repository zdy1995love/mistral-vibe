...
class VibeApp(App):  # noqa: PLR0904
    ...
    async def _compact_history(self, cmd_args: str = "", **kwargs: Any) -> None:
        ...
        if "--micro" in cmd_args.split():
            from vibe.core.microcompact import micro_compact

            threshold = self.agent_loop.config.get_active_model().auto_compact_threshold
            # Match MicroCompactMiddleware: when auto-compact is disabled
            # (threshold <= 0) the per-tool keep_last math collapses and
            # micro_compact would clear every eligible result. Bail early.
            if threshold <= 0:
                await self._mount_and_scroll(
                    ErrorMessage(
                        "Auto-compact is disabled (threshold <= 0); "
                        "micro-compact has nothing to gate on.",
                        collapsed=self._tools_collapsed,
                    )
                )
                return
            ...
            if tokens_freed > 0:
                ...
                # micro-compact creates no new session, so report via an info
                # message rather than CompactMessage.set_complete (which now
                # takes old/new session ids, not token counts).
                await self._mount_and_scroll(
                    UserCommandMessage(
                        f"Micro-compacted: ~{tokens_freed:,} tokens reclaimed "
                        f"({old_tokens:,} → {new_tokens:,} context tokens)."
                    )
                )
            else:
                await self._mount_and_scroll(
                    ErrorMessage(
                        "Nothing to micro-compact "
                        "(no old tool results found above threshold).",
                        collapsed=self._tools_collapsed,
                    )
                )
            return
